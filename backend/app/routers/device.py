...
@router.put('/{device_id}/params', response_model=DeviceResponse)
...
def batch_control(req: BatchControlRequest, db: Session = Depends(get_db)):
    ...
    db.commit()
    action_label = 'enabled' if req.status == 1 else 'disabled'
    return {'detail': f"batch {action_label}: {', '.join(changed)}"}
...
def get_device_health(db: Session = Depends(get_db)):
    ...
    result: list[DeviceHealthResponse] = []
